...
def main():
    ...
    for idx, path in enumerate(paths):
        # video, extension = os.path.splitext(os.path.basename(path))
        ...
        print('Testing', idx, video_name)
        save_path = os.path.join(args.output, f'{video_name}.mp4')

        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out = cv2.VideoWriter(save_path, fourcc, 8.0, (width * args.outscale, height * args.outscale))
        ...
        while True:
            ...
            pbar.update(1)
        end_time = time.time()
        print(f"esp_time = {end_time - start_time}")
        pbar.close()
        cap.release()
        out.release()
