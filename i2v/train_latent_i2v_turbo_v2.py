...
# source: https://github.com/Ji4chenLi/t2v-turbo
# coding=utf-8
...
def parse_args():
    ...
    parser.add_argument(
        "--pretrained_model_cfg_stu",
        type=str,
        default="configs/inference_i2v_512_v2.0.yaml",
        help="Pretrained Model Config.",
    )
    parser.add_argument(
        "--pretrained_model_path_stu",
        type=str,
        default="model_cache/VideoCrafter2_model.ckpt",
        help="Path to the pretrained model.",
    )
    ...
    return args
def save_results(save_dir, detail, index, decoded_imgs, scale=False):
    #if os.path.exists(save_dir) and len(os.listdir(save_dir)) > 0:
    #    print(f"Directory '{save_dir}' already contains files. Skipping save.")
    #    return

    if scale == True:
        decoded_imgs = (decoded_imgs / 2 + 0.5).clamp(0, 1)
    #save_dir = "output_images_cv2_with_scale"
    os.makedirs(save_dir, exist_ok=True)

    for i in range(decoded_imgs.size(0)):
        img_tensor = decoded_imgs[i].cpu()  # [C, H, W]
        img_np = img_tensor.mul(255).clamp(0, 255).byte().numpy()
        img_np = np.transpose(img_np, (1, 2, 0))
        if img_np.shape[2] == 3:
            img_np = cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)
            cv2.imwrite(os.path.join(save_dir, f"{detail}_{index}_image_{i:03d}.png"), img_np)
...
def main(args):
    ...
    config_stu = OmegaConf.load(args.pretrained_model_cfg_stu)
    model_config_stu = config_stu.pop("model", OmegaConf.create())

    pretrained_t2v_stu = instantiate_from_config(model_config_stu)
    pretrained_t2v_stu = load_model_checkpoint(
        pretrained_t2v_stu,
        args.pretrained_model_path_stu,
        strict=True
    )

    vae = pretrained_t2v_stu.first_stage_model
    vae_scale_factor = model_config_stu["params"]["scale_factor"]
    img_emb_stu = model_config_stu["params"]["img_cond_stage_config"]
    img_emb_stu = instantiate_from_config(img_emb_stu)
    img_emb_stu.load_state_dict(pretrained_t2v_stu.embedder.state_dict(), strict=True)

    img_proj_stu = model_config_stu["params"]["image_proj_stage_config"]
    img_proj_stu = instantiate_from_config(img_proj_stu)
    img_proj_stu.load_state_dict(pretrained_t2v_stu.image_proj_model.state_dict(), strict=True)

    text_encoder = pretrained_t2v_stu.cond_stage_model
    unet = pretrained_t2v_stu.model.diffusion_model
    #img_emb_stu = pretrained_t2v_stu.embedder
    #img_proj_stu = pretrained_t2v_stu.image_proj_model

    pretrained_t2v_stu = pretrained_t2v_stu.to(weight_dtype)
    pretrained_t2v_stu = pretrained_t2v_stu.to(device)
    ...
    # this part reqires large GPU memory. just this part!
    '''
    time_cond_proj_dim = (
        unet.time_cond_proj_dim
        if unet.time_cond_proj_dim is not None
        else args.unet_time_cond_proj_dim
    )
    unet_config_stu = model_config_stu["params"]["unet_config"]
    unet_config_stu["params"]["time_cond_proj_dim"] = time_cond_proj_dim
    if args.use_motion_cond:
        unet_config_stu["params"]["motion_cond_proj_dim"] = args.motion_cond_proj_dim
    unet_stu = instantiate_from_config(unet_config_stu)
    # load teacher_unet weights into unet
    if args.pretrained_unet_dir is not None:
        unet_stu.load_state_dict(
            torch.load(args.pretrained_unet_dir, map_location=device, weights_only=True)
        )
    else:
        unet_stu.load_state_dict(unet.state_dict(), strict=False)
    unet_stu.requires_grad_(True).train()
    '''
    unet_stu = pretrained_t2v_stu.model.diffusion_model
    unet_stu.requires_grad_(True).train()
    ...
    config_teacher = OmegaConf.load(args.pretrained_model_cfg_teacher)
    model_config_teacher = config_teacher.pop("model", OmegaConf.create())

    pretrained_t2v_teacher = instantiate_from_config(model_config_teacher)
    ...
    unet_teacher = pretrained_t2v_teacher.model.diffusion_model
    unet_teahcer = unet_teacher.to(device, weight_dtype)
    ...
    noise_scheduler = T2VTurboScheduler(
        linear_start=model_config_stu["params"]["linear_start"],
        linear_end=model_config_stu["params"]["linear_end"],
        prediction_type='v_prediction',
        rescale_betas_zero_snr=True
    )

    assert model_config_stu["params"]["linear_start"] == 0.00085
    assert model_config_stu["params"]["linear_end"] == 0.012
    ...
    if accelerator.unwrap_model(unet_stu).dtype != torch.float32:
        raise ValueError(
            f"Controlnet loaded as datatype {accelerator.unwrap_model(unet_stu).dtype}. {low_precision_error_string}"
        )

    #if accelerator.unwrap_model(img_emb_stu).dtype != torch.float32:
    #    raise ValueError(
    #        f"Controlnet loaded as datatype {accelerator.unwrap_model(img_emb_stu).dtype}. {low_precision_error_string}"
    #    )

    #if accelerator.unwrap_model(img_proj_stu).dtype != torch.float32:
    #    raise ValueError(
    #        f"Controlnet loaded as datatype {accelerator.unwrap_model(img_proj_stu).dtype}. {low_precision_error_string}"
    #    )
    ...
    #img_proj_stu.to("cpu", dtype=weight_dtype)
    #img_emb_stu.to("cpu", dtype=weight_dtype)
    if (
        accelerator.process_index in args.reward_train_processes
        and args.reward_scale > 0
    ) or (
    ...
    #initial_proj_weights = {k: v.clone().detach() for k, v in img_proj_stu.named_parameters()}
    #initial_emb_weights = {k: v.clone().detach() for k, v in img_emb_stu.named_parameters()}
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                unet_ = deepcopy(accelerator.unwrap_model(unet_stu))
                save_dir = os.path.join(output_dir, "unet.pt")
                torch.save(unet_.state_dict(), save_dir)

                #img_emb_ = deepcopy(accelerator.unwrap_model(img_emb_stu))
                #save_dir = os.path.join(output_dir, "img_emb.pt")
                #torch.save(img_emb_.state_dict(), save_dir)

                img_proj_ = deepcopy(accelerator.unwrap_model(img_proj_stu))
                save_dir = os.path.join(output_dir, "img_proj.pt")
                torch.save(img_proj_.state_dict(), save_dir)
    ...
    named_modules_dict = dict(unet_stu.named_modules())
    other_params = []

    #for n, p in unet_stu.named_parameters():
    for n, p in unet_stu.named_parameters():
        other_params.append(p)

    if args.train_img_emb:
        #img_emb_stu.requires_grad_(True)
        img_emb_stu.to(device, torch.float32)
        #other_params += [p for p in img_emb_stu.parameters() if p.requires_grad]

        img_proj_stu.requires_grad_(True)
        img_proj_stu.to(device, torch.float32)
        other_params += [p for p in img_proj_stu.parameters() if p.requires_grad]
    ...
    dataset = MP4LatentDataset(
        args.train_shards_path_or_url, latent_root_webvid=args.latent_root_webvid, latent_root_vidgen=args.latent_root_vidgen
    )
    if args.gpu_type == 'amd':
        train_dataloader = torch.utils.data.DataLoader(
            dataset,
            #batch_size=args.train_batch_size * accelerator.num_processes, # nvidia
            batch_size=args.train_batch_size, # amd
            shuffle=True,
    ...
    unet_stu, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
            unet_stu, optimizer, train_dataloader, lr_scheduler
        )
    unet_stu, img_proj_stu, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
                unet_stu, img_proj_stu, optimizer, train_dataloader, lr_scheduler
                )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet_stu): # not use
                if (
                    accelerator.process_index in args.video_rm_train_processes
                    and args.video_reward_scale > 0
                ...
                #uc_prompt_emb = batch["uc_prompt_emb"].to(device, weight_dtype)

                img_emb = batch["img_emb"].to(device, weight_dtype)
                #uc_img_emb = batch["uc_img_emb"].to(device, weight_dtype)

                img_emb_teacher = batch["img_emb_teacher"].to(device, weight_dtype)

                img_cat_cond = batch["img_cat_cond"].to(device, weight_dtype)
                ...
                if args.train_img_emb:
                    frame = batch["first_frame"].to(device, weight_dtype)
                    #img_emb = img_emb_stu(frame)
                    #img_emb = img_proj_stu(img_emb)
                ...
                #for name, param in img_emb_stu.named_parameters():
                #    count += 1
                ...
                #context = {"context": encoded_text, "fs": args.fps}

                context = {"context": torch.cat([prompt_emb, img_emb], dim=1), "fs": args.fps}
                #uc_context = {"context": torch.cat([uc_prompt_emb, uc_img_emb], dim=1), "fs": args.fps}

                zt_h = torch.cat([noisy_model_input, img_cat_cond], dim=1)
                context_teacher = {"context": torch.cat([prompt_emb, img_emb_teacher], dim=1), "fs": args.fps}
                ...
                #pred_x0 = pretrained_t2v_stu.predict_start_from_z_and_v(noisy_model_input, start_timesteps, noise_pred)

                #uc_pred_x0 = pretrained_t2v_stu.predict_start_from_z_and_v(noisy_model_input, start_timesteps, uc_noise_pred)
                #pred_x_0 = pred_x0 + w * (pred_x0 - uc_pred_x0)

                pred_x_0 = pretrained_t2v_stu.predict_start_from_z_and_v(noisy_model_input, start_timesteps, noise_pred)


                #test = pretrained_t2v.decode_first_stage(pred_x_0.to(pretrained_t2v.dtype))
                #test = (test / 2 + 0.5).clamp(0, 1)
                #save_results('output_images_cv2_with_scale', 'stu', index[0:1].item(), test[0:1,:,3], False)
                ...
                if accelerator.process_index in args.vlcd_processes:
                    ...
                    with torch.no_grad():
                        with torch.autocast("cuda"):
                            ...
                            #    sigma_schedule,
                            #)
                            cond_pred_x0 = pretrained_t2v_stu.predict_start_from_z_and_v(noisy_model_input, start_timesteps, cond_teacher_output)

                            cond_pred_noise = pretrained_t2v_stu.predict_eps_from_z_and_v(noisy_model_input, start_timesteps, cond_teacher_output)
                            ...
                            uncond_pred_x0 = pretrained_t2v_stu.predict_start_from_z_and_v(noisy_model_input, start_timesteps, uncond_teacher_output)

                            uncond_pred_noise = pretrained_t2v_stu.predict_eps_from_z_and_v(noisy_model_input, start_timesteps, uncond_teacher_output)
                    ...
                    with torch.no_grad():
                        with torch.autocast("cuda", dtype=weight_dtype):
                            ...
                            #    motion_cond=motion_gs_embedding,
                            #)
                            zt_h = torch.cat([x_prev, img_cat_cond], dim=1)

                            target_noise_pred = unet_teacher(
                                zt_h,
                                timesteps,
                                **context_teacher
                            )

                            pred_x_0 = pretrained_t2v_stu.predict_start_from_z_and_v(x_prev, start_timesteps, target_noise_pred)
                        ...
                        #print(timesteps, start_timesteps)
                        #test = pretrained_t2v.decode_first_stage(pred_x_0.to(pretrained_t2v.dtype))
                        #test = (test / 2 + 0.5).clamp(0, 1)
                        #save_results('output_images_cv2_with_scale', 'teacher', index[0:1].item(), test[0:1,:,3], False)
                ...
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(unet_stu.parameters(), args.max_grad_norm)
                    #if args.train_img_emb:
                    #    accelerator.clip_grad_norm_(img_emb_stu.parameters(), args.max_grad_norm)
                    #    accelerator.clip_grad_norm_(img_proj_stu.parameters(), args.max_grad_norm)
